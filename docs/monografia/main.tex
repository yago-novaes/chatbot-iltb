...
\begin{quadro}[!htbp]
  ...
  D10 & Avaliação com 1 especialista em ILTB usando TAM-AIN + rubrica clínica
        ...
        davis1989tam, ocaf2024}. \\ \hline
  D11 & Busca híbrida BM25 + denso (RRF) com \textit{reranking}
        \textit{cross-encoder} mGTE --- implementada no escopo deste trabalho
...
\begin{enumerate}[label=\textbf{TF\arabic*}]
  ...
  \item \textbf{LLM especializado em diretrizes brasileiras} --- avaliar a
        ...
        custo de \textit{fine-tuning} e a perda de atualização em tempo
        real \cite{teachingLLMsBrazil2026};
  \item \textbf{Simplificação arquitetural do \textit{vector store}} ---
        ...
        o corpus de 898 \textit{chunks} introduz regressão mensurável de
        \textit{recall}/latência. A literatura sugere que, para corpora
        pequenos ($< 10^4$ \textit{chunks}), a busca exaustiva
